...
from pyramid_apispec.annotations import activate
...
class ResourceMeta(type):
    def __new__(mcs, name, bases, attrs):
        ...
        methods = [
            each.lower()
            for each in getattr(klass, "methods", None) or flask.views.http_method_funcs
        ]
        for key, value in attrs.items():
            if key.lower() in methods:
                parents = [
                    getattr(parent, key) for parent in mro if hasattr(parent, key)
                ]
