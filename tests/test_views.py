...
from pyramid_apispec import doc, use_kwargs, marshal_with
...
class TestFunctionViews:
    ...
    def test_integration(self, app, client, models, schemas):
        @app.route("/")
        @use_kwargs(
            {"name": fields.Str(), "genre": fields.Str()}, location="querystring"
        )
        @marshal_with(schemas.BandSchema)
        def view(**kwargs):
...
class TestClassViews:
    def test_inheritance_unidirectional(self, app, client):
        ...
        assert (
            BaseResource.get.__apispec__["docs"][0].options[0]["description"]
            == "parent"
        )
        assert (
            ChildResource.get.__apispec__["docs"][0].options[0]["description"]
            == "child"
        )
